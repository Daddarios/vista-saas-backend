...
public class ChatController : ControllerBase
{
    ...
    public async Task<IActionResult> GetRaeume()
    {
        ...
        if (mandantId == null) return BadRequest(new { error = "X-Mandant-Id Header fehlt" });

        var raeume = await _db.ChatRaeume
            .AsNoTracking()
            .Where(r => !r.IstGeloescht && r.MandantId == mandantId.Value)
    ...
    public async Task<IActionResult> GetNachrichten(Guid raumId, [FromQuery] int page = 1, [FromQuery] int size = 50)
    {
        ...
        var raum = await _db.ChatRaeume
            .AsNoTracking()
            .FirstOrDefaultAsync(r => r.Id == raumId && r.MandantId == mandantId.Value && !r.IstGeloescht);
